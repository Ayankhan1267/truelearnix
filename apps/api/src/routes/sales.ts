...
// ── Shared: Activate order, create purchase/EMI, commission, notify ───────────
async function activateOrder(order: any, paymentMethod = 'manual', merchantOrderId?: string) {
  ...
  // emiDays: day offsets for each installment, e.g. [0, 15, 30, 45]
  ...
  if (isEmi && order.userId) {
    // Base for EMI = remaining after token (if token was already paid)
    ...
    // Look up salesperson's manager for manager commission
    ...
    // Credit installment 1 commissions immediately (already paid)
...
router.post('/orders/:id/verify-payment', ...guard, async (req: any, res) => {
  try {
    ...
    if (order.status === 'paid' && !order.commissionPaid) {
      await activateOrder(order, paymentMethod);
    }
