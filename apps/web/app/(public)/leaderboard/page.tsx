...
import { ALL_AFFILIATES } from '@/lib/affiliateData'
...
export default function LeaderboardPage() {
  ...
  const [all, setAll] = useState<any[]>(ALL_AFFILIATES)
  ...
  const list = filter === 'All' ? all : all.filter(a => a.tier === filter)
  const top3 = all.slice(0, 3)
  const totalPaid = all.reduce((s: number, a: any) => s + (a.monthlyEarnings ?? a.earned ?? 0), 0)
        ...
        <section className="relative py-14 px-4 overflow-hidden">
          ...
          <div className="max-w-5xl mx-auto relative text-center">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
              ...
              {[
                { icon: IndianRupee, val: formatINR(totalPaid), label: 'Paid This Month', color: '#34d399' },
                { icon: Users,       val: `${all.length}+`,     label: 'Active Partners', color: '#a78bfa' },
                { icon: TrendingUp,  val: top3[0] ? formatINR(top3[0].monthlyEarnings ?? top3[0].earned ?? 0) : '—', label: 'Top Earner', color: '#fbbf24' },
              ].map((s, i) => (
          ...
          {loading ? (
            ...
            <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.07)' }}>
              ...
              <div className="grid grid-cols-12 px-4 sm:px-6 py-3 text-[10px] font-black text-gray-600 uppercase tracking-widest"
                ...
                <div className="col-span-3 sm:col-span-3">Earned (Month)</div>
